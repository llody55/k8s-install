...
###
 ...
 # @LastEditTime: 2023-09-19 11:59:35
 # @FilePath: \K8S\组件包\k8s-部署\shell\k8s\01-rhel_init.sh
 # @Description: 这是一个系统初始化脚本,目前测试了centos7与openeuler
### 
...
set_install_soft (){
    ...
    then
        ...
        if [ $internet_conn -ne 0 ]; then
            ...
            print_color 32 "联网在线下载并安装软件包。。。"
            yum install -y makecache &>/dev/null
            yum update -y  &>/dev/null
            yum install -y epel-release &>/dev/null
            yum install -y gcc bash-completion wget vim screen lrzsz tree psmisc zip unzip bzip2 gdisk telnet net-tools sysstat iftop lsof iotop htop dstat &>/dev/null
        fi
...
update_kernel_version(){
    ...
    then
       ...
       print_color 31 "未测试的更新项，请手动查看当前系统的内核版本"
    fi      
}
...
log(){
    ...
    print_info_yellow "check  包含更新内核"
